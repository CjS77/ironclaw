/// <reference types="vitest/config" />
import tailwindcss from "@tailwindcss/vite";
import react from "@vitejs/plugin-react";
import { defineConfig } from "vitest/config";
import { dirname, resolve } from "node:path";
import { fileURLToPath } from "node:url";
import { storybookTest } from "@storybook/addon-vitest/vitest-plugin";
import { playwright } from "@vitest/browser-playwright";

const here = dirname(fileURLToPath(import.meta.url));
const rustStaticDir = resolve(here, "..", "static");

// Dev-server proxy target for the Reborn gateway. Honors the same
// REBORN_HOST/REBORN_PORT vars that `serve` and run-reborn-webui.sh use, so
// `REBORN_PORT=3001 pnpm dev` forwards to a backend on :3001. Defaults to
// 127.0.0.1:3000.
const backendTarget = `http://${process.env.REBORN_HOST ?? "127.0.0.1"}:${
  process.env.REBORN_PORT ?? "3000"
}`;

export default defineConfig({
  base: "/",
  plugins: [tailwindcss(), react()],
  publicDir: "public",
  resolve: {
    // Keep one React runtime without rewriting package ids through Node's
    // CommonJS resolver. Bare imports can now select each dependency's ESM
    // export, which lets Rolldown tree-shake unused code.
    dedupe: ["react", "react-dom"],
  },
  server: {
    host: "127.0.0.1",
    port: 5173,
    fs: {
      allow: [here, rustStaticDir],
    },
    proxy: {
      "/api/webchat/v2": backendTarget,
      "/api/reborn": backendTarget,
      "/auth": backendTarget,
      // Static SPA assets and self-hosted fonts are served locally from
      // `public/` (never proxied): `bypass` returning the request URL skips the
      // proxy and lets Vite's static pipeline serve it. This keeps the avatar,
      // logo, and fonts working even when the backend runs with
      // SKIP_FRONTEND_BUILD=1 (no embedded asset table). Same pattern the
      // storybook test project below uses; the keys stay present so the
      // root-paths.test.ts root-scoping guard still sees them.
      "/assets": { target: backendTarget, bypass: (req) => req.url },
      "/vendor": { target: backendTarget, bypass: (req) => req.url },
      "/wallet/connect": backendTarget,
      "/wallet-connect.js": backendTarget,
    },
  },
  build: {
    outDir: "dist",
    emptyOutDir: true,
    manifest: true,
    sourcemap: false,
    target: "es2022",
    rollupOptions: {
      input: {
        app: resolve(here, "index.html"),
        "wallet-connect": resolve(here, "src/wallet-connect.ts"),
      },
      external: ["@hot-labs/near-connect"],
      output: {
        entryFileNames: (chunk) =>
          chunk.name === "wallet-connect" ? "wallet-connect.js" : "assets/[name]-[hash].js",
      },
    },
  },
  // Two Vitest projects: the existing Node unit suite, plus the Storybook
  // story suite run in a headless Chromium via @storybook/addon-vitest.
  // See: https://storybook.js.org/docs/writing-tests/integrations/vitest-addon
  test: {
    projects: [
      {
        extends: true,
        test: {
          name: "unit",
          environment: "node",
          include: [
            "src/**/*.{test,spec}.{ts,tsx}",
            "src/pages/extensions/hooks/useExtensions-oauth.test.mjs",
          ],
          setupFiles: ["src/test/vm-tsx-setup.ts"],
        },
      },
      {
        extends: true,
        // `extends: true` inherits the top-level `server.proxy`, which forwards
        // `/assets` to the dev backend on :3000. In the story suite there is no
        // backend, so a Sidebar/GatewayLayout render requesting
        // `/assets/logo.png` would hit ECONNREFUSED and paint a broken logo even
        // while tests pass. A project-level `proxy: {}` cannot clear it (Vite
        // *merges* proxy records, so an empty object is a no-op), so override
        // the `/assets` entry with a `bypass` that always returns the request
        // URL — never proxied, served straight from `public/` (the real logo
        // ships at public/assets/logo.png). Stories must never reach a backend.
        server: {
          proxy: {
            "/assets": {
              target: "http://127.0.0.1:3000",
              bypass: (req) => req.url,
            },
          },
        },
        // Pre-bundle the deps the stories pull in so Vitest's browser Vite
        // server does not discover them mid-run, re-optimize, and reload —
        // that reload fails in-flight story modules and stalls the suite.
        optimizeDeps: {
          include: [
            "react",
            "react-dom",
            "react-dom/client",
            "react/jsx-runtime",
            "react/jsx-dev-runtime",
            "react-router",
            "@tanstack/react-query",
            "react-hot-toast",
            "qrcode",
          ],
        },
        plugins: [
          storybookTest({
            configDir: resolve(here, ".storybook"),
          }),
        ],
        test: {
          name: "storybook",
          browser: {
            enabled: true,
            headless: true,
            provider: playwright({}),
            instances: [{ browser: "chromium" }],
          },
        },
      },
    ],
  },
});
